...
```bash
docker pull nlmacamp/check_mk:1.5.0p12    (or nlmacamp/check_mk:latest)
```
...
If you want to change the version of check_mk, use `--build-arg CMK_VERSION_ARG=1.5.0p12 --build-arg CMK_DOWNLOADNR_ARG=38`.
...
https://<span></span>mathias-kettner.de\/support\/**1.5.0p12**\/check-mk-raw-**1.5.0p12**-el7-**38**.x86_64.rpm
...
```bash
docker build -t nlmacamp/check_mk github.com/viper0131/check_mk --build-arg CMK_VERSION_ARG=1.5.0p12 --build-arg CMK_DOWNLOADNR_ARG=38
```
...
3. Run this command (with correct container name and cmk version and downloadnr): `docker exec -it <container> /opt/update.sh <CMK_VERSION_ARG> <CMK_DOWNLOADNR_ARG>` (Ex. `docker exec -it check_mk /opt/update.sh 1.5.0p12 38`)
